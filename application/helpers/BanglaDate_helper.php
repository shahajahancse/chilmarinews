...
if (!function_exists('bangla_date_format')) {
   function bangla_date_format($item) { 
      $convertedDATE = '';
      if($item != ''){     
         $currentDate = date_format(date_create($item), "d M Y");
...
if (!function_exists('bangla_day')) {
   function bangla_day($item) { 
      $convertedDATE = '';
      if($item != ''){     
         $engDATE = array('Sat','Sun','Mon','Tue','Wed','Thu','Fri');
         $bangDATE = array('শনিবার','রবিবার','সোমবার','মঙ্গলবার','বুধবার','বৃহস্পতিবার','শুক্রবার' );
         $convertedDATE = str_replace($engDATE, $bangDATE, $item);
      }
      return $convertedDATE;
   }
}
